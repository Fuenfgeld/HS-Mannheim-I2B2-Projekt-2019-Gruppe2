...
class Kohortenabfrage():
    ...
    def umwandeln_in_fullname(abfrage, baum):
        ...
        for i in kriterien:
            ...
            for j in baum.knotenliste_mit_baum:
                breakflag = False
                for k in j:
                    if k.shortcode == i[1] or k.text == i[1]:
                        fullnames.append([i[0], k.fullname])
                        breakflag = True
                        found = True
                        break
                if breakflag == True:
    ...
    def __init__(self, kriterien, verknüpfungen, flag_push=True):
        ...
        self.df_hauptdia, self.df_nebendia = bl.umwandeln_in_sql_statement_und_df_hauptdia_nebendia_neu(self.kriterien,
                                                                                                        self.verknüpfungen)
